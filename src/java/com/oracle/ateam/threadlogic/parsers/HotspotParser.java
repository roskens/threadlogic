...
public class HotspotParser extends AbstractDumpParser {
  ...
  public HotspotParser(BufferedReader bis, Map threadStore, int lineCounter, boolean withCurrentTimeStamp,
    int startCounter, DateMatcher dm, boolean isNonHotspot) {
      this(bis, threadStore, lineCounter, withCurrentTimeStamp, startCounter, dm);
   ...
   */
   ...
   /**
   * check and parse manually the thread label for a dump originally from IBM
   * 
   * @param nameEntry
   *          the thread name line
   */
   private String[] doIBMParsing(String nameEntry) {
      ...
      // The thread would appear like following if generated by WLST for IBM dump:
      // "Thread-4" waiting for lock java.util.Timer$TimerImpl@71507150 WAITING native
    ...
    try {
      ...
      while (bis.ready() && count++ < 2) {        
        ...
        if (line != null) {
          ...
          if (index > 0) {            
            this.setJvmVersion(line.substring(index).trim().replaceAll(":", ""));
            return;
          }
        }
      }
